...
static void Sensor_GetFwVerRespNext(RCS_t *rcs)
{
    printf("Firmware  Version: %s \n",Sensor.st_SenInfo.FwVer);
}
...
static void Sensor_GetSenSNRespNext(RCS_t *rcs)
{
    printf("Sensor SN: %s \n",Sensor.st_SenInfo.SN);
}
...
static void Sensor_GetModelNameRespNext(RCS_t *rcs)
{
    printf("Modele Number : %s \n",Sensor.st_SenInfo.ModelName);
}
...
static void Sensor_GetProdNameRespNext(RCS_t *rcs)
{
    printf("Product name : %s \n",Sensor.st_SenInfo.ProdName );
}
...
static void Sensor_GetSenIDRespNext(RCS_t *rcs)
{
    printf("ID: 0X%02X,SubID:0X%02X \n",Sensor.st_SenInfo.SenID, Sensor.st_SenInfo.SubID);
}
...
static void Sensor_GetDataPackRespNext(RCS_t *rcs){

    Output("Reading : %d\n",Sensor.st_RunData.GasReading);
...
static void Sensor_ShowSenSum(RCS_t *rcs)
{
    printf("Sensor Nubmer : %d, Sensor Map =0X%0X \n",Sensor.st_SenInfo.MaximalSenNum,Sensor.st_SenInfo.InstalledSenMap );
}
...
const RCS_CmdNode SCDC_Sensor_RespTable[] = {
    //-----------------------------------------------
    SDCS_GET_MODEL_NAME,                 RCS_CMD_REV1,     Sensor_GetModelNameResp,        Sensor_GetFwVerRespNext,    
    SDCS_GET_PROD_NAME,                  RCS_CMD_REV1,     Sensor_GetProdNameResp,         Sensor_GetProdNameRespNext,    
    SDCS_GET_SENSOR_FWVER,               RCS_CMD_REV1,     Sensor_GetFwVerResp,            Sensor_GetFwVerRespNext,    
    SDCS_GET_SENSOR_SN,                  RCS_CMD_REV1,     Sensor_GetSenSNResp,            Sensor_GetSenSNRespNext,    
    SDCS_GET_SENSOR_SUM,                 RCS_CMD_REV1,     Sensor_GetSenSumResp,           NULL,    
    SDCS_GET_PROD_DATE,                  RCS_CMD_REV1,     Sensor_GetProdDataResp,         NULL,    
    SCDC_GET_DATA_PACK,                  RCS_CMD_REV1,     Sensor_GetDataPackResp,         Sensor_GetDataPackRespNext,    
