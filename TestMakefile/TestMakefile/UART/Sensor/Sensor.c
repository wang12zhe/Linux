...
void SCDC_SensorHost_Init(void)
{
...
static INT8U  Sensor_GetFwVer(void)
{
    INT8U ret;
    INT8U index =0;
    SensorComLock();
    ...
    do{
        ...
        ret = CommHandler_SCDCSensrResp(&SCDC_SensorResp);
        index ++;
    }while((ret == 0) &&(index <5));
    SensorComUnLock();
    if(index >=5){
...
static INT8U Sensor_GetSenSN(void)
{
    INT8U ret;
    INT8U index =0;
    SensorComLock();
...
static INT8U Sensor_GetModelName(void)
{
    INT8U ret;
    INT8U index =0;
    SensorComLock();
...
static INT8U Sensor_GetProdName(void)
{
    INT8U ret;
    INT8U index =0;
    SensorComLock();
...
static INT8U Sensor_SenSum(void)
{
    INT8U ret;
    INT8U index =0;
    SensorComLock();
...
INT8U Sensor_GetSenPara(INT8U SensorIndex,INT16U ParaMask)
{
    INT8U ret;
    INT8U index =0;
    SensorComLock();
    ...
    do{
        ...
        rcs->DataLen = 0;
        rcs-> DataBuf[rcs->DataLen ++] = SensorIndex;
        rcs-> DataBuf[rcs->DataLen ++] = (ParaMask >> 8) & 0XFF;
        rcs-> DataBuf[rcs->DataLen ++] = (ParaMask     ) & 0XFF;
        CommHandler_SCDCSensrHost(rcs);
        TM_Delay(DELAY_6MS);
        SCDC_SensorResp.PktIndex = index;
        ret = CommHandler_SCDCSensrResp(&SCDC_SensorResp);
